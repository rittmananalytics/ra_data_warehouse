...
FROM
{{ target.database}}.{{ var('stg_mailchimp_emailstitch_schema') }}.{{ var('stg_mailchimp_emailstitch_reports_email_activity_table') }})

where _sdc_batched_at = max_sdc_batched_at)
SELECT
  concat('{{ var('stg_mailchimp_email-id_prefix') }}',list_id) as list_id,
  concat('{{ var('stg_mailchimp_email-id_prefix') }}',campaign_id) as send_id,
  concat('{{ var('stg_mailchimp_email-id_prefix') }}',email_id) as contact_id,
  timestamp,
  action,
