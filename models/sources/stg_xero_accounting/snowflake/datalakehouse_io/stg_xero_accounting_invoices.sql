...
{% if var("stg_xero_accounting_etl") == 'datalakehouse_io' %}
 ...
 with source as (
   select *
   from {{ source('datalakehouse_xero_accounting','invoice') }}
 ),
renamed as (
     SELECT
       ...
       cast(inv_date as {{ dbt_utils.type_timestamp() }}) as invoice_created_at_ts,       
       cast(null as  {{ dbt_utils.type_timestamp() }}) as invoice_issue_at_ts,       
       cast(due_date as  {{ dbt_utils.type_timestamp() }}) as invoice_due_at_ts,      
       cast(null as  {{ dbt_utils.type_timestamp() }}) as invoice_sent_at_ts,
       cast(fully_paid_on_date as  {{ dbt_utils.type_timestamp() }}) as invoice_paid_at_ts,              
