...
renamed as (
  SELECT
    ...
    {{ cast() }} as project_id,
    {{ cast() }} as invoice_creator_users_id,
    {{ cast() }} as invoice_subject,
    date as invoice_created_at_ts,
     {{ cast(datatype='timestamp') }} as invoice_issue_at_ts,
    duedatestring as invoice_due_at_ts,
     {{ cast(datatype='timestamp') }} as invoice_sent_at_ts,
    fullypaidondate as invoice_paid_at_ts,
     {{ cast(datatype='timestamp') }} as invoice_period_start_at_ts,
     {{ cast(datatype='timestamp') }} as invoice_period_end_at_ts,
