...
renamed as (
  select * from (
    SELECT
      ...
      min(_sdc_received_at::TIMESTAMP) over (partition by id)  as ad_campaign_start_date,
      max(_sdc_received_at::TIMESTAMP) over (partition by id)  as ad_campaign_end_date,
