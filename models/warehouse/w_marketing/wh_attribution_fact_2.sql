        ...
        FROM (
          SELECT
            *,
            ...
            ROW_NUMBER() OVER (PARTITION BYblended_user_id ORDER BY session_start_ts) AS session_seq
          FROM (
            SELECT
...
add_time_decay_score AS (
  SELECT
    *,
    ...
    {{ iff() }} (conversion_session AND NOT TRUE,0,(COUNT(CASE WHEN NOT conversion_session OR TRUE THEN session_id END) over (PARTITION BYblended_user_id,DATE_TRUNC('day', CAST(session_start_ts AS DATE))))) AS sessions_within_day_to_conversion,
    {{ iff() }} (conversion_session AND NOT TRUE,0,div0 (pow(2, (days_before_conversion - 1)), COUNT(CASE WHEN NOT conversion_session OR TRUE THEN session_id END) over (PARTITION BYblended_user_id, DATE_TRUNC('day', CAST(session_start_ts AS DATE))))) AS weighting_split_by_days_sessions
  FROM
...
session_attrib_pct AS (
  SELECT
    * ,
    ...
    WHEN session_id = LAST_VALUE({{ iff() }}(is_within_attribution_lookback_window and (not conversion_session or {{ var('attribution_include_conversion_session') }}),session_id,null)  IGNORE NULLS) OVER (PARTITION BYblended_user_id, user_conversion_cycle ORDER BY session_start_ts ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) THEN 1
      ELSE 0
  ...
  {{ iff() }}(conversion_session and not {{ var('attribution_include_conversion_session') }},0,CASE
    WHEN session_id = LAST_VALUE({{ iff() }}(is_within_attribution_lookback_window and (not conversion_session or {{ var('attribution_include_conversion_session') }}) and is_paid_channel,session_id,null)  IGNORE NULLS) OVER (PARTITION BYblended_user_id, user_conversion_cycle ORDER BY session_start_ts ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) THEN 1
      ELSE 0
    END) AS last_paid_click_attrib_pct,
  {{ iff() }}(conversion_session and not {{ var('attribution_include_conversion_session') }},0,CASE
    WHEN session_id = FIRST_VALUE({{ iff() }}(is_within_attribution_lookback_window,session_id,null) IGNORE NULLS) OVER (PARTITION BYblended_user_id, user_conversion_cycle ORDER BY session_start_ts ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) THEN 1
      ELSE 0
  ...
  {{ iff() }}(conversion_session and not {{ var('attribution_include_conversion_session') }},0,CASE
    WHEN session_id = FIRST_VALUE({{ iff() }}(is_within_attribution_lookback_window  and (not conversion_session or {{ var('attribution_include_conversion_session') }}) and is_paid_channel,session_id,null) IGNORE NULLS) OVER (PARTITION BYblended_user_id, user_conversion_cycle ORDER BY session_start_ts ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) THEN 1
      ELSE 0
    END)
    AS first_paid_click_attrib_pct,
  {{ iff() }} (conversion_session AND NOT TRUE,0,
    {{ iff() }} (is_within_attribution_lookback_window,(div0 (1,(COUNT({{ iff() }}(is_within_attribution_lookback_window,session_id,NULL)) over (PARTITION BYblended_user_id,user_conversion_cycle ORDER BY session_start_ts rows BETWEEN unbounded preceding AND unbounded following) + 0))),0)
    ) AS even_click_attrib_pct,
  {{ iff() }} (conversion_session AND NOT TRUE,0,CASE WHEN is_within_attribution_time_decay_days_window THEN apportioned_time_decay_score / NULLIF((SUM(apportioned_time_decay_score) over (PARTITION BYblended_user_id, user_conversion_cycle)),0) END
