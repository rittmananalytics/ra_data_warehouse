...
converting_events as
    (
      SELECT
        e.blended_user_id,
        first_value(CASE WHEN event_type = 'subscribed' THEN session_id END) over (PARTITION BY e.blended_user_id order by e.event_ts ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as session_id,
...
converting_sessions_deduped as (
    SELECT
      ...
      MAX(CASE WHEN event_type = 'subscribed' THEN session_id END ) AS session_id,
