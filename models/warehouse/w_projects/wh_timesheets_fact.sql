...
SELECT
    ...
    min(timesheet_billing_date) over (partition by c.company_pk order by timesheet_billing_date) as first_company_timesheet_billing_date,
    max(timesheet_billing_date) over (partition by c.company_pk order by timesheet_billing_date) as last_company_timesheet_billing_date,
