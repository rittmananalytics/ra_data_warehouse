...
SELECT
    ...
    min(timesheet_billing_date) over (partition by c.company_pk RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as first_company_timesheet_billing_date,
    max(timesheet_billing_date) over (partition by c.company_pk RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as last_company_timesheet_billing_date,
